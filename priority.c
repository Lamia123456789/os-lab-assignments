...
void main()
{
	int n, i, j, pos, temp, *bt, *wt, *tat, *p, *pt,*at;
 ...
 for(int k=0; k<n; k++)
    ...
    printf("\n PROCESS \t PRIORITY \t arival TIME \t BURST TIME \t WAITING TIME \t TURNAROUND TIME \n");
    printf("--------------------------------------------------------------------------------\n");
    for(i=0; i<n; i++)
    {
        printf(" P%d \t\t %d  \t\t %d \t\t %d \t\t %d \t\t %d \n", p[i], pt[i], at[i],bt[i], wt[i], tat[i]);
    }
